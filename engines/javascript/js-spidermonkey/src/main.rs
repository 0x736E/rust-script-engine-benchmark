...
fn run(rt: Runtime) {
    ...
    let res = rt.evaluate_script(_global.handle(), sample, filename, lineno, rval.handle_mut());

    if res.is_ok() {
        let result = rval.get();

        println!("The answer is: {:?}", result.to_string());
    } else {
        eprintln!("Error evaluating script: {:?}", res.err().unwrap());
    }
}
